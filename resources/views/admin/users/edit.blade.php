...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.users.update", [$user->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('area') ? 'is-invalid' : '' }}" name="area_id" id="area_id">
                    @foreach($areas as $id => $entry)
                        <option value="{{ $id }}" {{ (old('area_id') ? old('area_id') : $user->area->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('area'))
            ...
            <div class="form-group">
                <label for="carrera_id">{{ trans('cruds.user.fields.carrera') }}</label>
                <select class="form-control select2 {{ $errors->has('carrera') ? 'is-invalid' : '' }}" name="carrera_id" id="carrera_id">
                    @foreach($carreras as $id => $entry)
                        <option value="{{ $id }}" {{ (old('carrera_id') ? old('carrera_id') : $user->carrera->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('carrera'))
                    <span class="text-danger">{{ $errors->first('carrera') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.user.fields.carrera_helper') }}</span>
            </div>
            <div class="form-group">
