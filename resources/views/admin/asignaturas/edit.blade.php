...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.asignatura.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.asignaturas.update", [$asignatura->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="nombre">{{ trans('cruds.asignatura.fields.nombre') }}</label>
                <input class="form-control {{ $errors->has('nombre') ? 'is-invalid' : '' }}" type="text" name="nombre" id="nombre" value="{{ old('nombre', $asignatura->nombre) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('carrera') ? 'is-invalid' : '' }}" name="carrera_id" id="carrera_id" required>
                    @foreach($carreras as $id => $entry)
                        <option value="{{ $id }}" {{ (old('carrera_id') ? old('carrera_id') : $asignatura->carrera->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
