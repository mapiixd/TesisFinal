...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.progresoDelProyecto.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.progreso-del-proyectos.update", [$progresoDelProyecto->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('proyecto') ? 'is-invalid' : '' }}" name="proyecto_id" id="proyecto_id" required>
                    @foreach($proyectos as $id => $entry)
                        <option value="{{ $id }}" {{ (old('proyecto_id') ? old('proyecto_id') : $progresoDelProyecto->proyecto->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('progreso') ? 'is-invalid' : '' }}" type="number" name="progreso" id="progreso" value="{{ old('progreso', $progresoDelProyecto->progreso) }}" step="1">
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('fecha_de_actualizacion') ? 'is-invalid' : '' }}" type="text" name="fecha_de_actualizacion" id="fecha_de_actualizacion" value="{{ old('fecha_de_actualizacion', $progresoDelProyecto->fecha_de_actualizacion) }}" required>
